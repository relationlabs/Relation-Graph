...
impl QueryResults {
    ...
    pub fn read(
        ...
        match format {
            ...
            QueryResultsFormat::Xml => results::xml_results::read_xml_results(reader),
            #[cfg(feature = "sparql-results-json")]
            QueryResultsFormat::Json => results::json_results::read_json_results(reader),
            QueryResultsFormat::Tsv => results::csv_results::read_tsv_results(reader),
            _ => Err(invalid_input_error(
                "SPARQL results format parsing is not implemented",
            )),
        }
    }
    ...
    ///
    /// ```
    /// use ic_graph_core::MemoryStore;
    /// use ic_graph_core::model::*;
    /// use ic_graph_core::sparql::QueryResultsFormat;
    ///
    ...
    pub fn write(
        ...
        match format {
            #[cfg(feature = "sparql-results-xml")]
            QueryResultsFormat::Xml => results::xml_results::write_xml_results(self, writer),
            #[cfg(feature = "sparql-results-json")]
            QueryResultsFormat::Json => results::json_results::write_json_results(self, writer),
            QueryResultsFormat::Csv => results::csv_results::write_csv_results(self, writer),
            QueryResultsFormat::Tsv => results::csv_results::write_tsv_results(self, writer),
            _ => Err(EvaluationError::QueryResultsFormat),
        }
    }
    ...
    /// ```
    /// use ic_graph_core::MemoryStore;
    /// use ic_graph_core::io::GraphFormat;
    /// use ic_graph_core::model::*;
    /// use std::io::Cursor;
    ///
...
/// ```
/// use ic_graph_core::MemoryStore;
/// use ic_graph_core::sparql::QueryResults;
///
/// let store = MemoryStore::new();
...
impl QuerySolutionIter {
    ...
    /// ```
    /// use ic_graph_core::MemoryStore;
    /// use ic_graph_core::sparql::{QueryResults, Variable};
    ///
    /// let store = MemoryStore::new();
...
///
/// ```
/// use ic_graph_core::sparql::Variable;
///
/// assert_eq!(
///     "?foo",
///     Variable::new("foo")?.to_string()
/// );
/// # Result::<_,ic_graph_core::sparql::VariableNameParseError>::Ok(())
/// ```
